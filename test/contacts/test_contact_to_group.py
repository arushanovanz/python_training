...
def test_remove_contact_from_group(db,app,orm):
    if len(db.get_group_list()) == 0:
        app.group.create(Group(name="test group"))
    ...
    contact_in_group = random.choice(orm.get_contacts_in_group(Group(id=group_id))).id
    app.contact.delete_contact_from_group(contact_in_group, group_id)
    assert db.get_contact_by_id(contact_in_group) in  orm.get_contacts_not_in_group(Group(id=group_id))